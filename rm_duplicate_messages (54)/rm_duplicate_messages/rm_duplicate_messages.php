...
class rm_duplicate_messages extends rcube_plugin
{
    ...
    function init ()
    {
        ...
        */
        ...
        if ($this->rc->task == 'mail' && ($this->rc->action == '' || $this->rc->action == 'list')) {
            ...
            * @return string   Путь к скину относительно каталога плагинов.
            */
            // Загружаем файл общей таблицы стилей CSS (скина) по - умолчанию для нашего плагина (skins / larry / rm_duplicate_messages.css).
    ...
    function msg_request()
    {
        ...
        */
        ...
        // Иначе если текущее сообщение это "msg1" то увеличим счётчики первого и второго сообщений.
        elseif ($key == "msg1") {
            // Увеличим счётчики первого и второго сообщений:
            // получим значение "msg1" и "msg2" из массива "cfg_rm_duplicate" по ссылке через &.
            $msg1 =&$cfg_rm_duplicate['msg_offset']['msg1'];
        ...
        if ($msg1 >= $msg_sum_uids - 1) {
            ...
            */
            ...
            // Номер текущей страницы.
            //$args .= "mailbox_page=" . $this->rc->storage->list_page . "\n";
            $args .= "mailbox_page=" . $cfg_rm_duplicate['mailbox_page'] . " ";
            ...
            //$a=0;
            //if($a==1){
				// Вызов функции корая выполняется на стороне клиента для циклической обработки писем.
    ...
    /**
    * Область видимости свойства или метода может быть определена путем использования следующих
    * ключевых слов в объявлении: public, protected или private.
    * Модификатор public (общедоступный) разрешает доступ к свойствам и методам класса отовсюду.
