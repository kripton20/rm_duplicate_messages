    ...
    function msg_request()
    {
        ...
        */
        ...
        foreach ($cfg_rm_duplicate['msg_offset'] as $key => $msg_offset) {
            ...
            //$data_subject = $MESSAGE->headers->subject;
            // Это вместо If (strpos($data_subject, ' '))
            //$cont = trim(rcube_mime::decode_header($header->$col, $header->charset));
            //$cont1 = trim(rcube_mime::decode_header($data_subject, $header->charset));
            $a=1;
            //$subject = trim(rcube_mime::decode_header($MESSAGE->headers->subject, $header->charset));
            $a=2;
            //$date = $MESSAGE->headers->date;
            //strtotime($date, $timezone = null)
            //$date_strtotime = rcube_utils::strtotime($MESSAGE->headers->date);
            $a=3;
//            /**
...
//            */
//            // В условии проверяем есть - ли пробел в строке темы сообщения.
//            If (strpos($data_subject, ' ')) {
//                // Поиск строки в подстроке.
//                // Разделим строку по пробелу.
//                $data_subject = explode(' ', $data_subject);
//                // В цикле перебираем массив и декодируем.
//                foreach ($data_subject as $k1 => $v1) {
//                    /**
//                    * substr — Возвращает подстроку в строке
//                    * Описание:
...
//                    * Эта функция возвращает строку string с удалёнными из начала и конца строки пробелами.
...
//                    */
//                    $data_subject = base64_decode($substr);
//                    // Запишем значение переменной 'subject3' в массив 'data_subject1'
//                    $data_subject_m[$k1] = $data_subject;
//                }
//                // Инициализируем переменную.
//                $subject = '';
//                // В цикле перебираем массив и склеим строки темы сообщения из массива в одну строку.
//                foreach ($data_subject_m as $k2 => $v2) {
//                    $subject .= $data_subject_m[$k2];
//                }
//            }
            /**
