...
class RoundcubeLogin
{
    ...
    */
    ...
    /**
    * Отладку можно включить, задав второй аргумент
    * в конструкторе TRUE.
    *
    * @var bool
    */
    ...
    */
    public function isLoggedIn()
    {
        ...
        if (!$this->rcLoginStatus) throw new RoundcubeLoginException("Невозможно определить статус входа в систему из-за технических проблем.<br />");
    ...
    */
    ...
    {
        if ($this->rcSessionID && $this->rcLoginStatus && !$forceUpdate) return;

        // Получить файл cookie с идентификатором текущего сеанса
        if ($_COOKIE['roundcube_sessid']) $this->rcSessionID = $_COOKIE['roundcube_sessid'];

        if ($_COOKIE['roundcube_sessauth']) $this->rcSessionAuth = $_COOKIE['roundcube_sessauth'];
        ...
        if (preg_match('/"request_token":"([^"]+)", /mi', $response, $m)) $this->lastToken = $m[1];

        if (preg_match('/<input.+name="_token".+value="([^"]+)"/mi', $response, $m)) $this->lastToken = $m[1]; // override previous token (if this one exists!)
    ...
    */
    private function sendRequest($path, $postData = false)
    {
        ...
        // Отладочное сообщение от запроса.
        ...
        while (!feof($fp)) {
            ...
            */
            $line = fgets($fp, 4096);
            // Не найден.
            if (preg_match('/^HTTP\/1\.\d\s+404\s+/', $line)) throw new RoundcubeLoginException("Установка Roundcube не найдена на '$path'");
