...
class RoundcubeLogin
{
    ...
    public function login($username, $password, $logout = TRUE)
    {
        ...
        while (!feof($fp)) {
            ...
            if (preg_match('/^Set-Cookie:.+sessauth=-del-;/mi', $line)) {
                ...
                header($line, FALSE);
                // Запишем отладочное сообщение в массив "debugStack[]": вызываем функцию "addDebug()" и передаём ей сообщение.
                $this->addDebug("LOGIN FAILED", "RC sent 'sessauth=-del-'; Комбинация логин/пароль неправильная." . "\r\n");
    ...
    */
    public function redirect($folders=NULL)
    {
        // Получаем адрес страницы для перенаправления:
        //   если переменная "$folders" равна "NULL" - значит при входе попадаем на главную страницу,
        //   иначе если переменная "$folders" не равна "NULL" - значит при входе попадаем в указанную
        //   папку и подпапку.
        $folders ? $rcPath = $this->rcPath . $folders : $rcPath = $this->rcPath;
