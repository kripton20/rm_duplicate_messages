...
class RoundcubeLogin
{
    ...
    */
    public function __construct($webmailPath, $enableDebug = FALSE, $writeLogEnabled = FALSE, $sentRedirectEnabled = FALSE)
    {
    ...
    private function updateLoginStatus()
    {
        ...
        while (!feof($fp)) {
            ...
            */
            ...
            // || (preg_match('/<input.+name = "_token".+value = "([ ^ "] + )">/i', $line, $m))
    ...
    */
    public function sendRequest($path, $postData = FALSE)
    {
        ...
        */
        ...
        // Иначе просто запишем отладочное сообщение в массив "debugStack[]": вызываем функцию "addDebug()" и передаём ей сообщение содержащее наш запрос.
        else {
    ...
    */
    public function login($username, $password, $logout = TRUE)
    {
		...
		*/
        ...
        //"task = login & _action = login & _timezone = 1 & _dstactive = 1 & _url = " .
        "_task=login&_action=login&_timezone=_default_&_url= " .
        // Логин и пароль.
        ...
        //. "&_task = mail"
        //. "&_action = login"
        //. "&_timezone = " . urlencode("Europe / Moscow")
        //. "&_url = "
    ...
    */
    public function logout()
    {
        ...
        //http://localhost / rc147 / ?_task = logout & _token = tHpHcVRsDHngAT97FQdaJ3xGlcheSdDS
        //$data = (($this->lastToken) ? "_token = ".$this->lastToken." & " : "")
        // Команда передаваемая приложению Roundcube для выполнения: действие и задача - "logout".
        //. "_action = logout & _task = logout";
        //?_task = logout & _token = VqYDNGm63ljT19fq7795eBhh0OUSe8Rw
        //$data = "?_task=logout&_token=" . (($this->lastToken) ? $this->lastToken : "");
