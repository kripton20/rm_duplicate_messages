    ...
    function msg_save_prefs()
    {
        ...
        if (isset($_POST['_user_prefs_null_save_log']) || isset($_POST['_user_prefs_null'])) {
            ...
            */
            ...
            $args .= "Start ";
            // Имя хоста, порт, папка расположения приложения.
            $args .= $_SERVER['HTTP_HOST'] . ":" . $_SERVER['SERVER_PORT'] . "/" . $server_folder . " ";
            ...
            // Определяем колличество сообщений поставленных в обработку "msg_sum_uids=".
            $args .= count($user_prefs['rm_duplicate_messages']['uids']) . " ";
    ...
    function msg_request()
    {
        ...
        */
        ...
        foreach ($cfg_rm_duplicate['msg_offset'] as $key => $msg_offset) {
            ...
            if ($MESSAGE->headers->in_reply_to != NULL) {
                // Получаем заголовок "Ответиь".
            ...
            */
            ...
            $msgs[$key] = array(
                ...
                'cc'=> $data_cc,
                // Заголовок "Ответиь".
                'replyto'=> $data_replyto,
                'in_reply_to'=> $data_in_reply_to,
        ...
        */
        ...
        if ($msg1 >= $msg_sum_uids - 1) {
            ...
            */
            ...
            // Почтовый ящик  "mailbox=".
            $args .= $this->rc->user->data['username'] . " ";
            // Текущая папка "mailbox_folder=".
            $args .= $folder . " ";
            // Номер текущей страницы "mailbox_page=".
            $args .= $this->rc->storage->list_page . " ";
            // Колличество обработанных сообщений "msg_marked=".
            $args .= $cfg_rm_duplicate['msg_marked'] . "\n";
            // Вызываем функцию записи лог - файла.
            $this->write_log_file($args);
            // Удалим ранее созданные наши записи (настройки поиска и обработки писем)
            // - в массиве пользовательских настроек 'prefs'.
