...
class RoundcubeLogin
{
    ...
    */
    ...
    {
        // Вызываем функцию "updateLoginStatus()".
        $this->updateLoginStatus();
    ...
    private function updateLoginStatus($forceUpdate = FALSE)
    {
        ...
        */
        ...
        if (preg_match('/"request_token":"([^"]+)", /mi', $response, $m)) {
            ...
            $this->lastToken = $m[1];
            // Получаем токен из страницы ответа сервера.
        ...
        while (!feof($fp)) {
            ...
            *              $matches[1] - часть строки, соответствующую первой подмаске и так далее.
            * @var flags   flags может быть комбинацией следующих флагов:
            *              PREG_OFFSET_CAPTURE - В случае, если этот флаг указан, для каждой найденной подстроки будет указана её позиция (в байтах)
            *              в исходной строке. Необходимо помнить, что этот флаг меняет формат возвращаемого массива matches в массив, каждый элемент которого
            *              содержит массив, содержащий в индексе с номером 0 найденную подстроку, а смещение этой подстроки в параметре subject - в индексе 1.
            ...
            */
            // В условиях проверяем полученные строки и с помощью регулярных выражений получаем необходимые данные.
            ...
            } elseif (preg_match('/<input.+name="_token".+value="([^"]+)"/mi', $line, $m)) {
    ...
    */
    private function sendRequest($path, $postData = FALSE)
    {
        ...
        */
        ...
        // Вернём значение переменной "response".
        //return $response;
        return $fp;
    }
    ...
    * Если включено, добавлен префикс "ssl://". Если установлено значение NULL,
    * значение берётся из переменной $_SERVER['HTTPS'].
