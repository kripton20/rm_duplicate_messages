...
class rm_duplicate_messages extends rcube_plugin
{
    ...
    function msg_save_prefs()
    {
        ...
        if (isset($_POST['_user_prefs_null_save_log'])) {
            ...
            // если передаётся параметр 'user_prefs_null' -
            // то массиву 'user_prefs['rm_duplicate_messages'] присвоим NULL'.
        }elseif (isset($_POST['_user_prefs_null'])) {
            // Удалим ранее созданные наши записи (настройки поиска и обработки писем)
            // - в массиве пользовательских настроек 'prefs'.
            $user_prefs['rm_duplicate_messages'] = NULL;
            // Функция - прослушиватель события работы функции 'msg_save_prefs' об удалении
            ...
            */
            ...
            // Порядок обрабатываемых сообщений: все сообщения или только выделенные.
            $args .= "msg_processing=" . $user_prefs['rm_duplicate_messages']['msg_processing'] . " ";
            // Режим обработки найденных дубликатов писем: отмечать, удалять.
            $args .= "msg_process_mode=" . $user_prefs['rm_duplicate_messages']['msg_process_mode'] . " ";
            // Режим работы плагина: через браузер или серверный вариант.
    ...
    function msg_request()
    {
        ...
        if ($cfg_rm_duplicate['msg_processing'] == 'msg_selected') {
            ...
            $msg_sum_uids = count($cfg_rm_duplicate['uids'][$folder]);
        }else {
        ...
        */
        ...
        foreach ($cfg_rm_duplicate['msg_offset'] as $key => $msg_offset) {
            ...
            if (isset($MESSAGE->headers->flags['DUBLIKAT'])) {
                ...
                // // При необходимости можно убрать комментарии и удалить наши поставленные флаги.
                ...
                // // * @param string $flag   Флаг, который нужно снять: SEEN, DELETED, RECENT, ANSWERED, DRAFT,
                // // * MDNSENT
                // // * @param string $folder Имя папки
                // // * @return bool          Статус операции
                // // * @ см. set_flag
                // // */
                // // Удалим наши поставленные флаги.
                // // $storage->unset_flag($msg_uid, 'DUBLIKAT', $folder, true);
                // // $storage->unset_flag($msg_uid, 'DELETED', $folder, true);
                // // Увеличим на единицу колличество обработанных сообщений.
                // // $msg_marked++;
                // // }elseif ($cfg_rm_duplicate['msg_process_mode'] == 'del') {
                // // Удаляем сообщение.
                // // $storage->delete_message($msg_uid, $folder);
                // // Увеличим на единицу колличество обработанных сообщений.
                // // $msg_marked++;
                // // }
        ...
        */
        ...
        if (strcasecmp($msgs['msg1']['date'], $msgs['msg2']['date']) == 0) {
            ...
            If (strcmp($msgs['msg1']['from'], $msgs['msg2']['from']) == 0
                ...
                //MDN (Message Disposition Notification, уведомление об Открытии Сообщения).
                && strcmp($msgs['msg1']['mdn_to'], $msgs['msg2']['mdn_to']) == 0) {
                // Сравниваем вложения сообщений:
                // В условии проверяем если длина массивов вложений первого и второго сообщений одинаковая
                // тогда выполняем сравнение массивов.
                if ($msgs_attachment['msg1'].count() == $msgs_attachment['msg2'].count()) {
                    /**
                    * Вычислим расхождение массивов:
                    * array_diff(array $array, array...$arrays):array
                    * Сравнивает array с одним или несколькими другими массивами и возвращает значения из array,
                    * которые отсутствуют во всех других массивах.
                    * @param array $array   Исходный массив.
                    * @param arrays $array  Массивы, с которыми идёт сравнение.
                    * @return array         Возвращает массив (array), содержащий элементы array, отсутствующие
                    *                       в любом из всех остальных массивов. Ключи в массиве array сохраняются.
                    */
                    // Вычислим расхождение массивов.
                    $result = array_diff($msgs_attachment['msg1'], $msgs_attachment['msg2']);
                    // В условии проверяем: если - 0 то массивы одинаковые.
                    if ($result.count() == 0) {
                        // Инициализируем и получим объект хранилище.
                        //$storage = $this->rc->get_storage();
        ...
        // Безусловный переход к следующему оператору за меткой.
        metka:
        ...
        if ($msg1 >= $msg_sum_uids - 1) {
            ...
            */
            ...
            $args .= $this->rc->user->data['username'] . "\t";
            // Текущая папка.
            $args .= "folder=" . $cfg_rm_duplicate['folder'] . "\t";
